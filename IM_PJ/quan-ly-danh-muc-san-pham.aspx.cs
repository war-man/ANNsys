...
namespace IM_PJ
{
    public partial class quan_ly_danh_muc_san_pham : System.Web.UI.Page
    {
        ...
        {
            var categories = CategoryController.GetByLevel(0).ToList();
        ...
        public void Loadtree(int UID, string username)
        {
            var categories = CategoryController.GetByParentID("",UID);
            if (categories.Count > 0)
            {
                html += "<ul>";
                foreach (var c in categories)
                {
                    var ui = CategoryController.GetByID(c.ID);
                    html += "<li data-jstree='{ \"opened\" : true}'>" + c.CategoryName + " - <span class=\"register-link\" onclick=\"editCategory('" + c.ID + "')\">Chi tiết</span> | <span class=\"register-link\" onclick=\"AddChildCategory('" + c.ID + "')\">Sản phẩm</span>";
                    Loadtree(c.ID, c.CategoryName);
                    html += "</li>";
                }
        ...
        {
            List<tbl_Category> a = new List<tbl_Category>();
            ...
            {
                a = CategoryController.GetBySearchLevel("", 0);
            }
            if (a.Count > 0)
            {
                pagingall(a);
        ...
        #region Paging
        public void pagingall(List<tbl_Category> acs)
        {
            int PageSize = 15;
