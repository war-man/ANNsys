...
namespace IM_PJ
{
    public partial class chuyen_ke : System.Web.UI.Page
    {
        ...
        public static List<Product> getProduct(string sku, int floor, int row, int shelf, int floorShelf)
        {
            ...
            return ProductController.GetProductShelf(filter, true);
        }
        ...
        protected void btnConfirm_Click(object sender, EventArgs e)
        {
            ...
            var floor = hdfFloor.Value.ToInt(0);
            var row = hdfRow.Value.ToInt(0);
            var shelf = hdfShelf.Value.ToInt(0);
            var floorShelf = hdfFloorShelf.Value.ToInt(0);

            ProductController.updateChangeShelf(productShelf, floor, row, shelf, floorShelf, acc.ID);
            PJUtils.ShowMessageBoxSwAlert("Chuyển kê thành công!", "s", true, Page);
        }
    }
}